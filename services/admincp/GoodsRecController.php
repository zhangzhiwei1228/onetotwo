...
class Admincp_GoodsRecController extends Admincp_Controller_Action
{
	...
	public function doList()
	{
		...
		$view->render('goods/rec/list.php');
	}

	public function doAdd()
	{
		if ($this->_request->isPost()) {
			$id = M('Goods_Rec')->insert($this->_request->getPosts());
			$this->redirect(isset($this->_request->ref) ? base64_decode($this->_request->ref) : 'action=list&cid=' . $this->_request->getPost('category_id'));
		}

		$view = $this->_initView();
		$view->render('goods/rec/input.php');
	}

	public function doEdit()
	{
		$data = M('Goods_Rec')->getById((int)$this->_request->id);
		if (!$data->exists()) {
			throw new Suco_Controller_Dispatcher_Exception('Not found.');
		}

		if ($this->_request->isPost()) {
			M('Goods_Rec')->updateById($this->_request->getPosts(), (int)$this->_request->id);
	...
	public function doGoods()
	{
		...
		$view->datalist = $select->fetchRows();
		$view->render('goods/rec/goods.php');
	}
	...
	public function doSelected()
	{
		...
		$view->render('goods/rec/selected.php');
	}
}