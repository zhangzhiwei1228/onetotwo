...
<div class="sui-box">
	...
	<form method="post" action="<?=$this->url('action=batch')?>" class="sui-datalist">
		<div class="sui-toolbar">
			...
			<a class="btn btn-default btn-sm" href="<?=$this->url('action=add&ref='.$this->_request->url)?>"> <i class="fa fa-plus-circle"></i> 添加专题</a>
		</div>
		<table width="100%" class="table table-striped" data-plugin="chk-group">
			...
			<tbody>
				...
				<tr>
					...
					<td><a href="<?=$this->url('action=edit&id=' . $row['id'].'&ref='.$this->_request->url)?>"><?=$row['theme']?></a></td>
					<td><?=$row['clicks_num']?> 次</td>
