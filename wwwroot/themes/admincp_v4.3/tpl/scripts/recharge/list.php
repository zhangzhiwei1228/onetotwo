		...
		<table width="100%" class="table table-striped" data-plugin="chk-group">
			...
			<tbody>
				...
				<tr>
					...
					<td><a href="<?=$this->url('controller=money&action=detail&id='.$row['user_id'])?>"><?=$this->highlight($row['username'], $this->_request->q)?></a></td>				<td><?=$row['create_time'] ? date(DATETIME_FORMAT, $row['create_time']) : 'N/A'?></td>
					...
					<a href="<?=$this->url('action=commit&id=' . $row['id'].'&ref='.$this->_request->url)?>" onclick="return confirm('确定要入账吗?')">入账</a>
