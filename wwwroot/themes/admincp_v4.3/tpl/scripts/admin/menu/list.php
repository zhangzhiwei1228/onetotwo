...
<div class="sui-box">
	...
	<form method="post" action="<?=$this->url('action=batch')?>" class="form-inline">
		<div class="sui-toolbar">
			...
			<a class="btn btn-default btn-sm" href="<?=$this->url('action=add&ref='.$this->_request->url)?>"> <i class="fa fa-plus-circle"></i> 创建菜单</a>
		</div>
		<table width="100%" class="table table-striped" data-plugin="chk-group">
			...
			<tbody data-plugin="dragsort">
				...
				<tr class="<?=$class?>" data-id="<?=$row['id']?>">
					...
					<td><a href="<?=$this->url('action=add&pid=' . $row['id'])?>">加子类</a> | <a href="<?=$this->url('action=edit&id=' . $row['id'])?>">编辑</a> | <a href="<?=$this->url('action=delete&id=' . $row['id'])?>" onclick="return confirm('确定要删除这条记录吗?')">删除</a></td>
				</tr>
